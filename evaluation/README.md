...
# Judge bằng LLM local (Ollama/vLLM/LM Studio — report không rời máy):
RCA_JUDGE_BASE_URL=http://localhost:11434/v1 RCA_JUDGE_MODEL=qwen2.5:32b \
python evaluation/scripts/eval_judge.py --db rca_eval.duckdb \
    --report <report.md> --record <eval_record.json>
...
| `scripts/eval_judge.py` | LLM-as-judge theo rubric 6 chiều — Claude API, LLM local qua endpoint OpenAI-compatible (`RCA_JUDGE_BASE_URL`), hoặc xuất prompt cho người chấm |
