...
When `rca-analyze` needs to resolve an open question — where is
...
4. **Vendor documentation (NotebookLM, Qualcomm/MTK corpus)** —
   ...
   series — checking the log and code first means there is already
   something concrete to check the document's claim against, rather than
...
5. **Ask the engineer** — when no rung above resolves a question this
   ...
   evidence gaps (see `checkpoint-format.md`). This rung is also one of
   the four never-bypassable gates: when the round's *recommended*
   direction itself depends on a question this rung had to reach,
   `rca-analyze` always halts here regardless of `manifest.json.autonomy`.
   The engineer's answer, when it comes, is formalized as a `redirect
   <information>` reply, recorded at `ENGINEER_PROVIDED` and carried into
   the next round (`rca-analyze/SKILL.md`'s Step 3/6).
6. **Prior cases and playbooks** (`.rca/knowledge/cases/` and
   `.rca/knowledge/playbooks/`, written by `rca-learn`) — a case
   ...
   for a conclusion** ("Cases and playbooks are hints, never evidence") — a
...
Hypotheses in `rca-analyze` are generated against an **observed**
failure point (rung 2, or a rung-1-guided query that hit), not derived from
a procedure diagram before anyone has looked at the log — spec-before-log
is exactly where an LLM invents most freely, and
code-binding-as-precondition is exactly where source is least likely to
exist.
